...
class Client(shared.SharedService.Client, Iface):
  ...
  def getdiffsize(self, sz):
    ...
    Parameters:
     - sz
    """
    self.send_getdiffsize(sz)
    return self.recv_getdiffsize()

  def send_getdiffsize(self, sz):
    self._oprot.writeMessageBegin('getdiffsize', TMessageType.CALL, self._seqid)
    args = getdiffsize_args()
    args.sz = sz
    args.write(self._oprot)
    self._oprot.writeMessageEnd()
    self._oprot.trans.flush()

  def recv_getdiffsize(self):
    iprot = self._iprot
    (fname, mtype, rseqid) = iprot.readMessageBegin()
    if mtype == TMessageType.EXCEPTION:
      x = TApplicationException()
      ...
      iprot.readMessageEnd()
      raise x
    result = getdiffsize_result()
    result.read(iprot)
    iprot.readMessageEnd()
    if result.success is not None:
      return result.success
    raise TApplicationException(TApplicationException.MISSING_RESULT, "getdiffsize failed: unknown result")
...
class Processor(shared.SharedService.Processor, Iface, TProcessor):
  def __init__(self, handler):
    ...
    self._processMap["getdiffsize"] = Processor.process_getdiffsize
  ...
  def process_getdiffsize(self, seqid, iprot, oprot):
    args = getdiffsize_args()
    args.read(iprot)
    iprot.readMessageEnd()
    result = getdiffsize_result()
    try:
      result.success = self._handler.getdiffsize(args.sz)
      msg_type = TMessageType.REPLY
    except (TTransport.TTransportException, KeyboardInterrupt, SystemExit):
      raise
    except Exception as ex:
      ...
      logging.exception(ex)
      result = TApplicationException(TApplicationException.INTERNAL_ERROR, 'Internal error')
    oprot.writeMessageBegin("getdiffsize", msg_type, seqid)
    result.write(oprot)
    oprot.writeMessageEnd()
    oprot.trans.flush()
...
class getdiffsize_args:
  ...
  def write(self, oprot):
    if oprot.__class__ == TBinaryProtocol.TBinaryProtocolAccelerated and self.thrift_spec is not None and fastbinary is not None:
      oprot.trans.write(fastbinary.encode_binary(self, (self.__class__, self.thrift_spec)))
      return
    oprot.writeStructBegin('getdiffsize_args')
    if self.sz is not None:
      oprot.writeFieldBegin('sz', TType.I32, 1)
      oprot.writeI32(self.sz)
      oprot.writeFieldEnd()
    oprot.writeFieldStop()
    oprot.writeStructEnd()
...
class getdiffsize_result:
  ...
  def write(self, oprot):
    if oprot.__class__ == TBinaryProtocol.TBinaryProtocolAccelerated and self.thrift_spec is not None and fastbinary is not None:
      oprot.trans.write(fastbinary.encode_binary(self, (self.__class__, self.thrift_spec)))
      return
    oprot.writeStructBegin('getdiffsize_result')
    if self.success is not None:
